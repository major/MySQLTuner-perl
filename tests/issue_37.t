...
"t_low\tEngine\tVersion\tRow_format\t10\tAvg_row_length\tData_length\tMax_data_length\tIndex_length\tData_free\t5",
    ...
    # Verify results
    ok(
        !exists $main::result{'PctAutoIncrement'}{'test_db.t_empty'},
        "test_db.t_empty should be skipped (empty table, AUTO_INCREMENT <= 1)"
    );
    ok(
        exists $main::result{'PctAutoIncrement'}{'test_db.t_full'},
        "test_db.t_full should have capacity calculated"
    );
    ok(
        exists $main::result{'PctAutoIncrement'}{'test_db.t_low'},
        "test_db.t_low should have capacity calculated"
    );

    ok(
        grep(
/Table 'test_db.t_full' has an autoincrement value near max capacity/,
            @bad_prints ),
        "Warning is printed for test_db.t_full"
    );
    ok(
        !grep( /Table 'test_db.t_empty'/, @bad_prints ),
        "No warning is printed for test_db.t_empty"
    );
    ok(
        !grep( /Table 'test_db.t_low'/, @bad_prints ),
        "No warning is printed for test_db.t_low"
    );
};
