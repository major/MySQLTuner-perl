...
subtest 'sys_db_filter_definitions' => sub {
    ...
    for my $q (@statement_analysis_queries) {
        like($q, qr/db IS NULL OR db NOT IN/i, "Query filters db column: $q");
    }
    ...
    for my $q (@table_stats_queries) {
        like($q, qr/(WHERE|AND)\s*\(?table_schema IS NULL OR table_schema NOT IN/i, "Query filters table_schema column: $q");
    }
};
